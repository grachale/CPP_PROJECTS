...
public:
    // constructor
    CIndex( const T_ & line, const C_ & comparer = C_ () )
            : c_Line            ( line ),
              c_Comparer        ( comparer )
    {
    }
    // search ()
    set <size_t> search ( const T_ & word) const
    {
        set <size_t> indexes = {};
        ...
        // 1 step
        vector<unsigned int> pi { 0 };

        unsigned int iNumber = 1, jNumber = 0;
        auto jIterator = word . begin ();
        auto iIterator = word . begin (); iIterator++;

        while ( iIterator != word . end () )
        {
            if ( !c_Comparer ( (*iIterator), (*jIterator) ) &&
                 !c_Comparer ( (*jIterator), (*iIterator) ) )
            {
                pi . push_back( jNumber + 1 );
                iNumber++; jNumber++;
                iIterator++; jIterator++;
            ...
            {
                    unsigned int temp = jNumber;
                    jNumber = pi . at ( jNumber - 1);
                    for ( ; temp > jNumber; --temp, --jIterator );
            }
        }


        // 2 step
        unsigned int kNumber = 0, lNumber = 0;
        auto kIterator = c_Line . begin ();
        auto lIterator = word . begin ();

        while ( kIterator != c_Line . end () )
        {
            if ( !c_Comparer ( (*lIterator), (*kIterator) ) &&
                 !c_Comparer ( (*kIterator), (*lIterator) )  )
            {
                kIterator++; kNumber++;
                lIterator++; lNumber++;
                if ( lNumber == lengthOfWord )
                {
                    indexes.insert( kNumber - lNumber);
                    lNumber = 0;
                    lIterator = word.begin();

                    kNumber = kNumber - (lengthOfWord - 1);
                    for (unsigned int i = 0; i < lengthOfWord - 1; ++i)
                        kIterator--;
                }
            } else if ( lNumber == 0 )
            {
                    kNumber++; kIterator++;
                    if ( kNumber == c_Line . size () - 1 )
                        return indexes;
            } else
                {
                        unsigned int temp = lNumber;
                        lNumber = pi . at ( lNumber - 1 );
                        for ( ; temp > lNumber; --lIterator, --temp );
                }
            }
        return indexes;
    }
