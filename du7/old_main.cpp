...
public:
    ...
    set <size_t> search ( const T_ & word) const
    {
        ...
        for ( auto it = c_Line . begin (); it != c_Line . end (); ++it, ++index )
        {
            if ( !c_Comparer ((*it), (* word . begin () ) ) && !c_Comparer ( (* word . begin () ), (*it) ) )
            {
                auto itSecond = it;
                bool isFound = true;
                for ( auto it2 = word . begin (); it2 != word . end () && itSecond != c_Line . end () && isFound; ++ it2, ++itSecond )
                {
                    if ( c_Comparer ((* itSecond ), (* it2 ) ) || c_Comparer ( (* it2 ), (* itSecond ) ) )
                        isFound = false;
                    if ( ++it2 != word . end () && ++itSecond == c_Line . end () )
                        isFound = false;
                    else
                    {
                        --it2; --itSecond;
